...
type bound_variable_type = {
	var_name: string, var_dependencies: string[], var_binding: () => any, var_trigger: (variable_name: string) => void
};

type bound_variable_map = {
	[bound_variable_name: string]: bound_variable_type
};

var bound_variables: bound_variable_map = {

};

function bind_var(var_name: string, var_dependencies: string[], var_binding: () => any, var_trigger: (variable_name: string) => void = function () { }) {
	bound_variables[var_name] = { var_name, var_dependencies, var_binding, var_trigger };
	updated_variable(var_name);
}

function updated_variable(variable_name: string): void {
...
function find_affected_variables(variable_name: string): string[] {
	var affected_variables: string[] = [];
...
function get_var_value(var_name: string) {

	var bound_variable = bound_variables[var_name];

	if (bound_variable === undefined) return undefined;

	var var_dependencies = bound_variable.var_dependencies;
	for (var var_dependency of var_dependencies) {
		if (var_dependency == var_name) continue; // skip same
		if (get_var_value(var_dependency) === undefined) return undefined;
	}
...
function test_cases() {
	...
	function trigger_message(bound_variable_name: string) {
